...
# =====================================================================
    ...
    with aba[0]:
        ...
        acao_lead = None  # (tipo, lead_id, valor) — só é executada FORA do loop
            ...
            with st.expander(f"#{lead['id']} — {lead['nome']} ({lead['servico']})"):
                ...
                with c1:
                    ...
                    if novo_status != lead["status"]:
                        acao_lead = ("status", lead["id"], novo_status)
                with c2:
                    if st.button("🗑️ Excluir", key=f"del_lead_{lead['id']}"):
                        acao_lead = ("excluir", lead["id"], None)

        # Executa a ação (se houver) somente após o loop terminar de renderizar
        if acao_lead:
            tipo, lead_id, valor = acao_lead
            if tipo == "status":
                db.update_lead_status(lead_id, valor)
            elif tipo == "excluir":
    ...
    with aba[2]:
        ...
        item_excluir = None  # id do item — só é excluído FORA do loop
        ...
        # Executa a exclusão (se houver) somente após o loop terminar de renderizar
        if item_excluir:
            if os.path.exists(item_excluir["caminho_imagem"]):
                os.remove(item_excluir["caminho_imagem"])
            db.delete_portfolio_item(item_excluir["id"])
            st.rerun()
